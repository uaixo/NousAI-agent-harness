...
export function applyWebRuntime(ctx: Context, config: Config, identity: WebRuntimeIdentity): void {
  ...
  if (config.printUrl || handoffBrowser) {
    ...
    const announceReady = (): void => {
      const webUrl = localWebUrl(ctx, identity.pluginName)
      ...
      if (handoffBrowser) {
        ...
        void internals.openBrowser(webUrl).catch((error: unknown) => {
          ...
          console.error(`${identity.pluginName}: could not open the default browser because ${reason}; visit ${webUrl} manually`)
        })
      }
    }
