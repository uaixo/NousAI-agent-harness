...
A NousAI deployment gets its browser branding from this plugin: the NousAI marks in the sidebar and conversation-hero brand slots, and no DeepSeek-branded testing notice on first run. It fills `sidebar.brand.mark`, `sidebar.brand.name`, and `conversation.hero.brand.mark` (outside the official build the stock UI falls back to its DeepSeek fish and a build label in the sidebar, and to the fish in the hero) and shadows the stock `settings.onboarding` welcome step with one that completes immediately. Shadowing is reversible: the stock entry returns if this plugin unloads. Vendor onboarding, the provider API-key step, stays untouched: it names the model vendor, not the product.
...
The browser half registers three brand-slot occupants — `sidebar.brand.mark`, `sidebar.brand.name`, and `conversation.hero.brand.mark` — rendering the NousAI marks imported from `ui-primitives`, which the NousAI build serves as the NousAI brand module, so the slot occupants and every direct consumer stay one artwork. The hero occupant is load-bearing: `ui-brand-official` registers only the two sidebar slots, so without this occupant the hero would render `ui-conversation`'s own animated DeepSeek fish. It also shadows the stock `settings.onboarding` cell id `welcome-notice` (registered by [`ui-settings-models`](../ui-settings-models/README.md) at the default priority) with a step at priority −1 that completes immediately and renders nothing. All registrations dispose with the fiber.
...
| — | No runtime invariant companion is published: the brand-slot occupants and the onboarding cell are registry-disposed with the fiber, and this package holds no mutable state of its own to audit |
