  ...
  private emitSystem(prompt: string, anchor: SessionFormatEvent, context: SessionFormatMigrationContext): void {
    ...
    context.emitEvent(canonicalizeTransformedEvent({
      ...
      data: {
        ...
        message: {
          id,
          // The runtime-context projection's recorded producer name, so migrated heads
          // stay owned by it (see .agents/notes/implemented/feature/2026-08-15-runtime-context-producer-name.md).
          role: 'system', source: { kind: 'plugin', plugin: 'Runtime context' },
