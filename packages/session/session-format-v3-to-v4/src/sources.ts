...
const RELEASED_SAME_NAME_PRODUCERS: ReadonlySet<string> = new Set([
  // This fork recorded the runtime-context projection under this producer name,
  // which is also the kind it writes now.
  'Runtime context',
...
function producerKind(plugin: string, role: SessionFormatJsonValue | undefined): string {
  // Both producer names this repository has recorded for the system-prompt surface.
  if ((plugin === '@deepseek-ai/dsh-system-prompt' || plugin === 'Runtime context') && role === 'system') return 'system-prompt'
