...
describe('rewritePluginSource', () => {
  ...
  it('lifts the producer name this fork recorded onto both surfaces it wrote', () => {
    expect(rewritePluginSource({ kind: 'plugin', plugin: 'Runtime context' }, 1, 'system')).toEqual({ kind: 'system-prompt' })
    expect(rewritePluginSource({ kind: 'plugin', plugin: 'Runtime context' }, 1, 'user')).toEqual({ kind: 'Runtime context' })
    expect(rewritePluginSource({ kind: 'plugin', plugin: 'Runtime context', form: 'snapshot', sections: [] }, 1, 'user'))
      .toEqual({ kind: 'Runtime context', form: 'snapshot', sections: [] })
  })
