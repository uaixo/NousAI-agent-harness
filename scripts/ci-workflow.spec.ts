...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    // serial-windows: default-branch-only standby (main on this fork), self-hosted, non-blocking, lives in ci-master.
...
describe('Weighted approval workflow', () => {
  it('publishes from the trusted default branch after pull request and review updates', () => {
    ...
    expect(job).toMatchObject({
      // The publisher is guarded to the upstream repository on this fork: the
      // reviewer weights the score uses are upstream's.
...
describe('Issue lifecycle workflow', () => {
  it('allocates lifecycle runners only for events that can change the board, and only on the upstream repository', () => {
    ...
    // The Project board and the Issue App the job writes to exist only on the
    // upstream repository; on this fork the job skips instead of failing every
    // pull request on a missing token.
  ...
  it('mints Project credentials only after preflight and always revalidates current metadata', () => {
    ...
    // The policy script reads .github/issue-management/config.json, which
    // names the upstream repository; this fork guards the job to it.
