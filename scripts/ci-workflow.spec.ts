...
describe('Weighted approval workflow', () => {
  it('publishes from the trusted default branch after pull request and review updates', () => {
    ...
    expect(job).toMatchObject({
      // The publisher is guarded to the upstream repository on this fork (its
      // reviewer weights are upstream's); the event condition stays upstream's.
      if: "github.repository == 'deepseek-harness/deepseek-harness' && (github.event_name != 'workflow_run' || github.event.workflow_run.conclusion == 'success')",
