...
describe('CI workflow', () => {
  ...
  it('keeps required Wine and split native Windows jobs with failover, plus a master-only standby', () => {
    ...
    // wine-apt-cache: default-branch-only, seeds the Wine apt cache, lives in
    // ci-master. This fork's default branch is main, so the standby jobs that
    // upstream pins to master follow main here.
    expect(wineAptCache.if).toBe("github.event_name == 'push' && github.ref == 'refs/heads/main'")
    expect(wineAptCache['runs-on']).toBe('ubuntu-latest')

    // serial-windows: default-branch-only standby, self-hosted, non-blocking, lives in ci-master.
  ...
  it('exempts push from cancellation in ci-master, so one master merge does not cancel the running drill', () => {
    ...
    for (const name of ['serial-linux-selfhosted', 'serial-windows']) {
      ...
      // Both stay default-branch-push-only (main on this fork); that is what
      // makes the push carve-out safe.
