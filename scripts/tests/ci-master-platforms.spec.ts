...
// main is this fork's default branch; upstream schedules the same carriers on master.
const defaultBranchPush = "github.event_name == 'push' && github.ref == 'refs/heads/main'"
...
describe('default-branch-only platform scheduling', () => {
  ...
  it('runs all three deferred carriers on default-branch pushes with fail-loud API credentials', () => {
  ...
  it('runs Wine once on hosted default-branch CI and seeds its own apt cache', () => {
    const master = workflow('ci-master.yml')
    const wine = master.jobs.windows!
    expect(wine).toMatchObject({ if: defaultBranchPush, 'runs-on': 'ubuntu-latest' })
