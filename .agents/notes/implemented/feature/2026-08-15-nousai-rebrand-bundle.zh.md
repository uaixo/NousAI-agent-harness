...
- **模型可见身份。** 补丁以 `includeHarnessIdentity: false`、NousAI 角色设定前缀与原生的 cwd 角色设定后缀重述 `system-prompt` 行；胶水注册 NousAI 措辞的 `harness:source` / `app:web-surface` 段与 `DSH_WEB_URL` 描述。
...
- NousAI dist 是第二个前端构建（`pnpm run build:web:nousai`），刻意不并入 `pnpm run build`：可选项不进默认出货，代价是构建运行之前页面按请求返回 404（启动时没有提示）。
