...
- 只要来源仅携带插件 id，UI 中的生产者名称就呈现为包名形态（`dsh-compaction-basic`、`dsh-session-title-llm`）。这是拒绝客户端名称表的代价；想要更好标签的生产者必须在来源字段中记录该标签，[运行时上下文生产者名称](2026-08-15-runtime-context-producer-name.zh.md)就是这样做的。
