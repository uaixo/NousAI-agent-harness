...
- 该名称随录制 fixture、期望文件以及每个写出它的生产者一起变动；单元 spec、benchmark 与快照 harness 种子数据中那些既不会到达 `isOwned` 也不会渲染成行的合成来源保持原作者写法（上游编写的文件保留上游名称），因为改动它们不会改变任何可观察行为，只会扩大每次上游同步的合并面。引用上游名称的上游 note 描述的是上游；本 note 是 fork 记录值的权威来源。
