...
- **Shell swap.** `apps/web-nousai` (`@deepseek-ai/dsh-web-frontend-nousai`) rebuilds the stock shell with the NousAI page identity and — the load-bearing move — serves the NousAI marks as the `@deepseek-ai/dsh-client-ui-primitives` platform module via a build-time alias, so every direct consumer resolves one NousAI artwork with zero UI-plugin forks; the sidebar wordmark and conversation hero mark render through the generic browser-brand slots, which `dsh-client-ui-nousai-brand` fills with occupants built on that same module. The kernel boot page's `HARNESS` literal is swapped by a build transform because it is deliberately outside every plugin seam (shell self-sufficiency). The bundle's patch disables the stock `web-runtime` and `ui-brand-official` rows (the official occupants would race the NousAI ones for the single-kind brand slots) and its glue mounts `applyWebRuntime` — extracted into `dsh-web-app` as a shared, identity-parametrized helper — with the NousAI dist and wording.
...
- **Model-facing identity.** The patch restates the `system-prompt` row with `includeHarnessIdentity: false`, a NousAI persona prefix, and the stock cwd persona suffix, and the glue registers NousAI-worded `harness:source` / `app:web-surface` sections and `DSH_WEB_URL` description.
...
- The NousAI dist is a second frontend build (`pnpm run build:web:nousai`), deliberately not chained into `pnpm run build`: opt-ins stay out of shipped defaults, at the cost of request-time 404s (no startup hint) until that build runs.
