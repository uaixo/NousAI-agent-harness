...
- The name moves with recorded fixtures, expectation files, and every producer that writes it; synthetic sources inside unit specs, benchmarks, and snapshot-harness seeds that never reach `isOwned` or a rendered row stay as authored (upstream-authored files keep upstream's name), because moving them changes no observed behavior and only widens the merge surface of each upstream sync. Upstream notes that quote upstream's name describe upstream; this note is the fork's authority for the recorded value.
