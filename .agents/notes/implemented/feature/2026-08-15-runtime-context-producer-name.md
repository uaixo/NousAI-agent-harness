...
The name is the durable source kind [`runtime-context.ts`](../../../../packages/core/agent-loop/src/runtime-context.ts) writes, and a build that knows nothing more about a kind renders it verbatim, so one string is both the human-facing producer name and the identity `isOwned` matches to find the snapshot it must supersede. Upstream's `runtime-context` kind stays declared beside it and `isOwned` matches both, so a log written by an upstream build keeps its retained snapshot instead of gaining a second one. The added member carries `@persistenceAttribution`: a reader without this producer preserves the message and its JSON, and the kind imposes no validation, replay, or authority requirement. That qualification is what makes the addition a [same-version persistence change](../../../../docs/persistence-changes/2026-09-24-fork-runtime-context-kind.md) instead of a Session format bump. Two other producers restate the value: the [v2→v3 session migration](../../../../packages/session/session-format-v2-to-v3/src/migration.ts) labels the system heads it synthesizes with it, and the [v3→v4 lift](../../../../packages/session/session-format-v3-to-v4/src/sources.ts) carries recorded logs naming it onto this kind.

This is the remedy the [context-source marks decision](../../archived/feature/2026-08-04-web-context-source-and-steer-marks.md) named for exactly this row: a producer that wants a better label records one in its source fields. It stays a producer-side change on purpose. The client resolves producer names from the durable log alone and keeps no table of plugin ids, so a rename must never need a client release, and a resumed or foreign log must project the same way as a live one.
...
- `packages/core/agent-loop` unit coverage pins the recorded source on a fresh snapshot and the `isOwned` replay path that restores, supersedes, and clears a retained one, under both the written kind and upstream's.
...
- The name is durable data. A log written by an upstream build records `runtime-context`, which `isOwned` also matches, so that session keeps its retained snapshot and its row keeps rendering upstream's name. Released Session JSONL follows adjacent migration, which never rewrites a committed row, so no migration touches a recorded value; the v3→v4 lift maps both recorded producer names onto a format-4 kind as it publishes the successor.
...
- The name moves with recorded fixtures, expectation files, every producer that writes it, and every seed that feeds the live projection or renderer (Web e2e histories, benchmark workloads, snapshot-harness seeds), because an unowned seed makes `isOwned` append a fresh snapshot instead of superseding the seeded one. Synthetic sources in unit specs that only compare objects with each other stay as authored (upstream-authored files keep upstream's name): moving them changes no observed behavior and only widens the merge surface of each upstream sync. Upstream notes that quote upstream's name describe upstream; this note is the fork's authority for the recorded value.
