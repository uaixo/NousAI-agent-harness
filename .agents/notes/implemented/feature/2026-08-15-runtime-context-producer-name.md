...
The name is one constant in [`runtime-context.ts`](../../../../packages/core/agent-loop/src/runtime-context.ts) serving two roles at once: the human-facing producer name the client renders verbatim, and this projection's durable identity, which `isOwned` matches to find the snapshot it must supersede. Both roles move together by construction — the projection reads one string — and every recorded fixture carrying the old value moves with it. Two other producers restate the same value and move with it: the [v2→v3 session migration](../../../../packages/session/session-format-v2-to-v3/src/migration.ts) labels the system heads it synthesizes with it so the projection owns them after migration, and the [client fixture connection](../../../../packages/client/connection/src/client/fixture.ts) emits it for the prompt it emulates.
